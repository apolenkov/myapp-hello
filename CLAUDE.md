...
| `POST /v1/auth/register` | `{ accessToken }` 201 (public, validated DTO)  |
| `POST /v1/auth/login`    | `{ accessToken }` 200 (public, validated DTO)  |
...
1. **Create S3 bucket** — Yandex Object Storage (or any S3-compatible provider)
   - Current: bucket `myapp-hello`, endpoint `https://storage.yandexcloud.net`, region `ru-central1`
